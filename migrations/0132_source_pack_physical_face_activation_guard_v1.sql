...
INSERT INTO producer_receipt (
    ...
    'receipt_cimmich_source_pack_physical_face_activation_guard_v1',
    'system', 'cimmich-source-pack-physical-face-activation-guard', 'v1',
    now(), now(),
    encode(digest('cimmich-source-pack-physical-face-activation-guard-v1', 'sha256'), 'hex'),
    'release-safe'
    ...
    SELECT count(*) AS member_count, count(*) FILTER (
      WHERE EXISTS (
        ...
        JOIN current_face_identity identity
          ON identity.face_id = identity_member.face_id
        JOIN face_observation face ON face.face_id = reference_member.face_id
        ...
        WHERE reference_member.face_id = member.face_id
          ...
          AND (face.observation_origin <> 'manual_user' OR EXISTS (
            ...
            WHERE lifecycle.face_id = identity.face_id
              ...
              AND (reference.reference_kind <> 'face'
                OR lifecycle.vector_digest = reference.vector_digest)
          ))
      )
    ) AS trusted_count
    FROM unnest(
