...
-- Migration 0079's cimmich_probable_same_photo_derivative returned NULL when
-- the asset pair produced no row (missing/inactive asset or a NULL asset id):
-- the scalar SQL function then yields no row, so every
-- `NOT cimmich_probable_same_photo_derivative(...)` guard evaluated to NULL
-- and silently dropped the candidate instead of keeping it. Migration 0079 is
-- checksummed and immutable, so this forward migration re-issues the function
-- with the same conservative body but an explicit `false` for an empty pair:
-- a pair we cannot inspect is not a probable derivative.
...
AS $$
  WITH pack AS (
    ...
    LEFT JOIN immich_asset_projection left_projection
      ON left_projection.cimmich_asset_id = left_asset.asset_id
      AND left_projection.state = 'active'
    LEFT JOIN immich_asset_projection right_projection
      ON right_projection.cimmich_asset_id = right_asset.asset_id
      AND right_projection.state = 'active'
...
IS 'Conservative audit-only independence guard for exact files, exact export metadata, or multi-face geometry-preserving derivatives in one governed embedding space. An empty or uninspectable pair is explicitly not a probable derivative (false, never NULL).';
