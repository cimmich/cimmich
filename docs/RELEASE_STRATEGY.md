...
The current maintained candidate is `v1.1.0-community-preview.23` on schema
156/patch 1. Community Preview 18 remains immutable on schema 142/patch 1;
later preview numbers must never move or replace an existing tag.
...
`v1.1.0-community-preview.23` means the exact current self-contained Community
