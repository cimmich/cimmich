...
| Archive Health         | Open one first-class destination for exact copies, possible duplicates, Folder Check and Backup Check; start with the folders most affected by cross-folder duplicate evidence and compare aligned file metadata in compact columns. | Categories and detailed evidence load only when selected; recommendations remain Review only; an unconfigured or unsafe backup target fails closed; no result grants deletion authority. | `archive-integrity-layout.spec.ts`; `archive-folder-comparison.spec.ts`; `archive-backup-scanner.test.mjs`; all seven browser journeys. |
