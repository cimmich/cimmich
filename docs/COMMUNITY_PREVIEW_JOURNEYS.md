...
Updated: 2026-08-06
Candidate: `v1.1.0-community-preview.3`
...
| Library setup          | Connect to Immich, preview the exact scope, import it and understand what still needs review.                                                                            | Credentials are not shown again; permission rejection stays before import; interrupted import resumes; unnamed people are explicitly held for review rather than labelled as an error.                     | `CimmichImmichSetup.spec.ts`; `immich-onboarding-journey.mjs`; companion lifecycle acceptance.          |
