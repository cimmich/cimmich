...
Updated: 2026-08-19
Candidate: `v1.1.0-community-preview.18`
...
| People                 | Browse people visually, filter People and Person photos by exact privacy, tags/labels, Places, Events and Things, understand Face/Head/Body/Presence evidence and make owner-controlled corrections. | Protected facets lead through the viewing-mode boundary instead of appearing empty; ambiguous or conflicting identity operations retain manual correction and do not invent or automatically accept a match. | `people-*.spec.ts`; `explore-facets.test.mjs`; `run_synthetic_acceptance.sh`; signed-in collection/detail walkthrough.      |
| Matching and Refresh   | Confirm or correct a Person in Identity → Checks, preserve difficult appearances as Head or Body, run Refresh matches for that Person and review the New matches or Possible mistags that return. | Empty pools say there is nothing new; an unavailable calibrated matcher or refresh failure exposes a retryable state without changing accepted identity; separate SourcePack proposals remain held until their own activation gate; no result is accepted automatically. | `person-match-refresh.test.mjs`; `source-pack-*.test.mjs`; `face-matching-*.test.mjs`; signed-in Person Checks walkthrough. |
...
This matrix defines Preview acceptance. Other Immich versions, native Windows,
Internet-facing deployment and multi-user operation remain outside this
preview's tested boundary; identity acceptance always remains with the owner.
