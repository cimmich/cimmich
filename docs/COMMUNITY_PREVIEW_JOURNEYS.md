...
Updated: 2026-08-19
Candidate: `v1.1.0-community-preview.17`
...
| Matching and Refresh   | Confirm or correct a Person in Identity → Checks, preserve difficult appearances as Head or Body, run Refresh matches for that Person and review the New matches or Possible mistags that return. | Empty pools say there is nothing new; insufficient or worse reference proposals remain held; provider, evaluation and refresh failures keep the current matcher and expose a retryable state; no result is accepted automatically. | `person-match-refresh.test.mjs`; `source-pack-*.test.mjs`; `face-matching-*.test.mjs`; signed-in Person Checks walkthrough. |
