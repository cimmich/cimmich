...
| Archive Health       | archive-integrity route and backup-scan client                                                                                       | exact copies, possible-duplicate groups, inline preservation recommendation and read-only backup scan/proof | fingerprints/provenance durable or reproducible; scan state and recommendations derived         |
| Folder Check         | persistent Manage navigation plus archive-integrity `mode=folder`; path chooser, folder browser action and `ArchiveFolderComparison` | complete direct-folder inventory joined to exact and visual-duplicate topology                              | inventory and overlap are derived read models; no media or matcher authority                    |
...
Folder Check is a first-class navigation mode rather than a state hidden inside
Possible duplicates. Its sidebar link and tab open `mode=folder`; operators can
submit a path directly or enter the folder browser with preserved
`cimmichContext` and launch **Check this folder** from the current path. Existing
folder evidence links route to the same mode.
