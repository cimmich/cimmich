...
> [!IMPORTANT]
> This document follows the **Community Preview 24 candidate** (migration-ledger
...
| Discover (Experimental; off by default) | `routes/(user)/cimmich/discover/+page.svelte`, `CimmichMemoryGraph`; owner-enabled canvas-first mixed-node analysis workspace with semantic/collision-aware labels, independent topology/relationship toggles, spacing, lenses, on-demand Insights/selection rails, focus, path tracing, search, type filters, node drag and pan/zoom/fit; compact Person mode defaults to People-only points and separates Life periods, Events, Trips, Activities, Places and Things into explicit context views rendered as enclosures | when enabled, one visibility-projected `/v1/discover/memory-graph` read over accepted shared photos, current typed connection facts, context relations and hierarchy; shared client-side kind and Event/Life-period/Trip/Activity filtering, deterministic force layout, semantic label selection, bounded graph analysis and shortest-path projection | reads durable recorded facts but creates no truth; disabling the experiment hides its entry points and clears its projection without deleting recorded facts; Person-local viewpoint projection, layout, label density/collision suppression, Insights, lenses, paths, weights and evidence labels are bounded derivations |
...
| Person workspace     | `people/[personName]/+page.svelte`; Details, Connections, searchable typed connection/context add, local Web with optional Person-line labels, atomic Home/Employer/Group hub editor, suggestion cards, identity navigation, coverage, candidate review, split, names and presentation components                                                                                                                                                                                                           | Person assets/profile/names/presentation; typed fact/type/modifier routes for Person, Place and Thing connections; one idempotent `/v1/connection-hubs:record` command; Event-family context create/link; identity audit, match refresh and merge/split                                                                                  | profile, relationship and modifier catalogues, append-only typed facts, hub command receipts, suggestion decisions, Event-family context links and accepted evidence are durable; unconfirmed leads, graph layout, coverage, candidates and Smart Split are derived                              |
...
| Places/Things/Events | route shells plus `CimmichContextBrowser`, shared context hero framing, `CimmichEntityMediaActions` and map/plan components; Add media sources for privacy-projected human folders, Immich albums, recent library and wider/nearest-fallback GPS media; Folder/Album container drill-in presents a zero-selected thumbnail review with individual or explicit select-all choice; searchable Place movement and explicit GPS-from-Place action                                                                                            | context entity, typed fact, asset, revision-checked cover plus normalized crop, plan and geocoding routes; paged Immich folder/album search projected through active Cimmich visibility, GPS candidates, supported Immich asset metadata update for selected source IDs                                                                                                        | typed entities, hierarchy, links, cover framing and decisions durable; opening a folder/album is derived and non-selecting, candidate selection remains non-writing until explicit attachment; Context framing is presentation-only; Place movement changes Cimmich associations only; GPS update changes Immich metadata only and never source files |
...
| Archive Health       | one archive-integrity route and shared command bar for exact, variants, folder, rotation, missing-file and backup modes; `ArchiveFolderComparison`, `ArchiveRotationReview`, `ArchiveMissingFiles`, photo backup scanner and database backup manager                                                                                                                                                                                                                                                        | exact copies, possible-duplicate groups, direct-folder overlap, bounded Immich visual rotation leads, two-complete-run catalogue absence, explicit binding retirement, reversible correction commands, read-only photo backup comparison and scheduled database protection                                                               | fingerprints, missing-file command receipts, correction decisions, database backup policy and artifact receipts durable; folder inventory, candidate queues, photo scan state and recommendations derived                                                                                        |
...
- Person-scoped `Add to` entry for existing or newly created Events, Life
  periods, Trips, Activities, Places and Things;
- searchable existing-context and relationship selectors; Person-to-Place and
  Person-to-Thing additions use append-only typed connection facts, while the
  Event family retains participant context relations;
- reciprocal context reads project Person-to-Place and Person-to-Thing facts
  from the Place or Thing side without manufacturing a second persisted edge;
- append-only relationship-context events attach an existing Place, Event or
  Thing to a Person-to-Person fact; context detail projects both endpoints and
  Discover uses the context as a hub without manufacturing Person-to-context
  facts;
- privacy-projected mixed-node discovery derived from shared accepted photos,
  current context relations and hierarchy, with no inferred relationship claim;
- explicit cover decisions with a normalized presentation-only hero crop saved
  and undone atomically with the selected cover; and
...
The nearby picker sorts in-radius results first and may retain the closest
visible media beyond the selected 100 m–50 km boundary. Moving selected media
between Places changes Cimmich asset associations without changing GPS. The
separate GPS-from-Place command writes the selected point coordinates through
Immich's supported asset metadata API; it does not rewrite source files.
...
Missing-file management is based only on absence from complete supported Immich
catalogue reads. The first successful complete run that does not observe a
previously projected asset advances its projection to `suspected_missing`; a
second advances it to `missing`. A failed or interrupted inventory never calls
the completion function and therefore cannot advance either state. Immich's
`isOffline` and `isTrashed` fields are retained as source metadata but are not
absence authority. Archive Health exposes both evidence states and permits an
explicit, replay-safe removal command only for `missing` projections whose
source binding is also `missing`. The command supersedes only the selected
binding and tombstones the Cimmich asset only when no active, offline or missing
binding remains.
...
library-wide duplicate topology, deduplicates source and counterpart assets,
aggregates other folders by shared source-photo count and preserves the existing
byte-evidence classification. An unmatched folder file is labelled only as
absent from current duplicate evidence.
