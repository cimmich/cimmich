...
export const openCvSFaceRecognizerVersion =
  "cimmich.opencv-sface-recognizer.v1";
...
const requiredText = (value, label) => {
  ...
  if (!normalized) {
    throw recognizerError(
      ...
      `SFace recognizer requires ${label}`,
    );
  }
  return normalized;
};

export const createOpenCvSFaceRecognizer = ({
  detectorModelPath,
  ...
  if (
    ...
    throw recognizerError(
      "LOCAL_RECOGNIZER_CONFIG_INVALID",
      "SFace output limit is invalid",
    );
  }

  return {
    manifest,
    schemaVersion: openCvSFaceRecognizerVersion,
    async recognize({
      assetId,
      bytes,
      observations,
      ...
      if (!Buffer.isBuffer(bytes) || !bytes.length) {
        throw recognizerError(
          ...
          "SFace recognizer requires encoded image bytes",
        );
      }
      if (
        ...
        throw recognizerError(
          "LOCAL_RECOGNIZER_INPUT_INVALID",
          "SFace recognizer requires bounded observations",
        );
      }
      ...
      if (!Number.isInteger(effectiveTimeoutMs) || effectiveTimeoutMs < 1_000) {
        throw recognizerError(
          "LOCAL_RECOGNIZER_CONFIG_INVALID",
          "SFace command budget is invalid",
        );
      }
      let fileManifest;
      try {
        ...
        throw recognizerError(
          "LOCAL_RECOGNIZER_CONFIG_INVALID",
          "SFace manifest could not be read",
        );
      }
      const validatedFileManifest =
        validateRecognitionProviderManifest(fileManifest);
      if (
        validatedFileManifest.providerConfigDigest !==
        manifest.providerConfigDigest
      ) {
        throw recognizerError(
          "LOCAL_RECOGNIZER_CONFIG_INVALID",
          "SFace manifest changed after worker configuration",
        );
      }
            ...
            finish(() =>
              reject(
                recognizerError(
                  ...
                  "SFace output exceeded its bound",
                ),
              ),
            );
            return;
          ...
          finish(() =>
            reject(
              recognizerError(
                ...
                "SFace recognizer could not start",
              ),
            ),
          ),
        );
          ...
          try {
            ...
            finish(() =>
              reject(
                recognizerError(
                  "LOCAL_RECOGNIZER_OUTPUT_INVALID",
                  "SFace recognizer returned invalid output",
                ),
              ),
            );
            return;
          ...
          if (!Array.isArray(result?.packets)) {
            finish(() =>
              reject(
                recognizerError(
                  "LOCAL_RECOGNIZER_OUTPUT_INVALID",
                  "SFace recognizer omitted packets",
                ),
              ),
            );
            return;
          }
          const packets = result.packets.map((packet) => ({
            ...packet,
            schemaVersion: recognitionObservationSchemaVersion,
          }));
          finish(() => resolve(packets));
        });
