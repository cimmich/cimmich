...
vector proof.
Schema 133 adds an immutable, Person-scoped matcher-photo refresh generation.
An explicit refresh or a newly split Person rebuilds that Person's current
Prime photos, searches unassigned physical Faces and Faces assigned to People
marked Needs attention with the calibrated score/margin policy, and writes
human-review-only New-match proposals.
Legacy observations with no measured quality score are admitted at the
conservative Prime quality floor instead of being misclassified as measured
...
Schema 135 prevents a bounded XMP sidecar region from borrowing an intersecting
neighbour's Face embedding, makes accepted-identity audits use the canonical
physical Face, and append-only quarantines historic vectors that strongly match
an intersecting neighbour while contradicting their own canonical Face. It does
...
Schema 140 stores one current revision-bound analysis for each generated
summary tier (`smart` and `enhanced`). The row contains validated visual facts
and exact source/model/configuration lineage, not final display prose. The read
projection compiles those facts with current accepted People and Context so
