#!/usr/bin/env python3
"""Build a local-only Cimmich COCO17 Body-pose provider manifest."""
...
SCHEMA = "cimmich.body-pose-provider.v1"
...
def build(args: argparse.Namespace) -> dict:
    ...
    if not artifact.is_file():
        raise ValueError("Pose checkpoint is unavailable")
    core = {
        "execution": {
            "device": args.device,
            "network": "forbidden",
        ...
        "pose": {
            ...
            "jointSchema": "coco17",
            "keypointThreshold": args.keypoint_threshold,
        ...
        "provider": {
            "providerId": "ultralytics-yolo-pose",
            "versionId": "v1",
        },
        "resources": {
    ...
    return {**core, "poseConfigDigest": digest(core)}
...
def main() -> int:
    ...
    parser.add_argument("--keypoint-threshold", type=float, default=0.20)
