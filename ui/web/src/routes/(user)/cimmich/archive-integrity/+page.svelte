<script lang="ts">
  ...
  let mode = $state<'exact' | 'variants'>('exact');
  let error = $state('');
  ...
  <div class="mx-auto w-full max-w-7xl space-y-6 px-4 pt-4 pb-16 sm:px-6 lg:px-8">
    ...
    >
      ...
      <div class="mt-4 grid gap-3 md:grid-cols-3">
        {#each [['Canonical plan', 'Choose a preferred version only after byte, metadata and evidence review.'], ['Backup proof', 'Verify canonical media and sidecars across independent destinations.'], ['Sidecar export', 'Merge owner-approved Cimmich truth into staged, round-trip-tested XMP.']] as item (item[0])}
