<script lang="ts">
  ...
  let { oncomplete = () => undefined, onreadiness = () => undefined, scope }: Props = $props();
  ...
  const unresolvedCount = $derived(
    clusters.filter((cluster) => cluster.resolution.state === 'unresolved' || cluster.resolution.state === 'stale')
      .length,
  );
...
<section
  ...
  class="mt-6 border-t border-indigo-200 pt-6 dark:border-indigo-900"
>
  ...
  {/if}
  {#if notice}
    <p class="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-950">{notice}</p>
  {/if}
              ...
              <p class="mt-3 text-sm/6">
                {cluster.resolution.action === 'existing_person' || cluster.resolution.action === 'create_person'
                  ...
                  : cluster.resolution.action === 'later'
                    ? 'Held for later; import remains blocked for this cluster.'
