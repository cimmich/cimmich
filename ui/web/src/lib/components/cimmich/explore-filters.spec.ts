...
describe('Cimmich Explore filter URLs', () => {
  ...
  it('carries People filters into a Person workspace without losing its identity selector', () => {
    expect(
      withCimmichExploreFilters('/cimmich/people/Maya?personId=person-maya', {
        eventIds: [],
        futureDates: false,
    ...
    ).toBe('/cimmich/people/Maya?personId=person-maya&privacy=private&label=label-restricted');
  });

  it('round-trips the exact future-date review scope', () => {
    const source = new URL('http://cimmich.local/cimmich/people/Cedar?personId=person-cedar&mode=photos&future=1');
    const filters = cimmichExploreFiltersFromUrl(source);
