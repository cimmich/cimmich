<script lang="ts">
  ...
  interface Props {
    ...
    contextGroups: ArchiveVariantGroup[];
    detailedGroupCount: number;
    error: string;
    filter: 'all' | ArchiveVariantClassification;
    groups: ArchiveVariantGroup[];
    loading: boolean;
    loaded: boolean;
    moreAvailable: boolean;
    onFilter: (filter: 'all' | ArchiveVariantClassification) => void;
    onLoadMore: () => void;
  }
...
<section class="space-y-4" aria-labelledby="variant-groups-title">
  <div class="flex flex-wrap items-center justify-between gap-4 px-1">
    <div>
      ...
      <p class="mt-0.5 text-sm text-gray-600 dark:text-gray-300">
        <strong>{countLabel(allGroupCount, 'group')} found</strong> · {countLabel(detailedGroupCount, 'group')} detailed.
        Comparisons load as needed. A visual match is a review lead, not deletion proof.
      </p>
    </div>
    <div class="flex flex-wrap gap-1 rounded-full bg-gray-100 p-1 dark:bg-gray-900" aria-label="Filter variants">
      {#each [['all', 'All'], ['verified_variant', 'Different files'], ['verified_exact', 'Exact copies'], ['similarity_candidate', 'Unverified']] as option (option[0])}
        <button
          ...
          onclick={() => onFilter(option[0] as typeof filter)}>{option[1]}</button
        >
      {/each}
    </div>
  </div>
  ...
  {:else}
    {#each groups as group (group.duplicateId)}
      ...
      >
        ...
        <div class="grid gap-4 p-4 sm:grid-cols-2 xl:grid-cols-3">
          {#each group.assets as asset, assetIndex (asset.id)}
            ...
            {@const folderContext = archiveVariantFolderContext(contextGroups, asset)}
