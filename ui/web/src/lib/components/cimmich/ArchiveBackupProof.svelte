  ...
  <article class="rounded-3xl border border-gray-200 bg-white p-5 dark:border-immich-dark-gray dark:bg-immich-dark-bg">
    ...
    {:else if targets.length === 0}
      <div class="flex items-start gap-4">
        ...
        >
        <div>
          <h3 class="font-semibold">No independent destination is connected</h3>
          <p class="mt-1 max-w-3xl text-sm/6 text-gray-600 dark:text-gray-300">
            Mount a backup read-only for Cimmich and give it a storage-domain identity different from the archive disk.
            Cimmich will not scan an arbitrary server path or count another folder on the archive disk as a backup.
          </p>
        </div>
      </div>
    {:else}
      ...
      {#if scan.status === 'queued' || scan.status === 'scanning'}
        <div class="p-5">
          <p class="text-sm font-semibold">
            {scan.progress.filesHashed.toLocaleString()} of {scan.progress.filesDiscovered.toLocaleString()} discovered files
            hashed
          </p>
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
