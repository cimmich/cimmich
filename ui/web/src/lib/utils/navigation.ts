...
function currentUrlWithoutAsset() {
  ...
  if (isPhotosRoute(page.route.id)) {
    const cimmichPersonId = page.url.searchParams.get('cimmichPersonId');
    const cimmichPersonName = page.url.searchParams.get('cimmichPersonName');
    if (cimmichPersonId && cimmichPersonName) {
      return Route.cimmichPerson({ name: cimmichPersonName, personId: cimmichPersonId });
    }
...
async function navigateAssetGridRoute(route: AssetGridRoute, options?: NavOptions) {
  ...
  if (!assetId && (page.url.searchParams.has('cimmichPersonId') || page.url.searchParams.has('cimmichPetId'))) {
    const next = currentUrlWithoutAsset();
    if (next !== currentUrl() || options?.forceNavigate) {
      await goto(next, options);
    }
    return;
  }
